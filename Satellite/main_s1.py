...
def _env_str(name, default):
    raw_value = os.getenv(name)
    return default if raw_value in (None, "") else raw_value


def _env_datetime(name, default):
    raw_value = os.getenv(name)
    return default if raw_value in (None, "") else datetime.fromisoformat(raw_value)

# ======================== 全局配置（需与S2协商确认）========================
# 1. 时间配置（T0时刻：仿真起始时间，与S2保持一致）
T0_UTC = datetime(2026, 3, 5, 8, 0, 0)  # 示例：2026-03-05 8:00:00 UTC
T0_UTC = _env_datetime("S1_T0_UTC", T0_UTC)
SIM_DURATION_SEC = _env_int("S1_SIM_DURATION_SEC", 600)  # 仿真总时长（10分钟）
TIME_STEP_SEC = _env_int("S1_TIME_STEP_SEC", 1)  # 时间步长（1秒/帧）
...
OBS_LAT = _env_float("S1_OBS_LAT", OBS_LAT)
OBS_LON = _env_float("S1_OBS_LON", OBS_LON)
OBS_ELE = _env_float("S1_OBS_ELE", OBS_ELE)

# 3. 卫星筛选配置
MIN_ALT_DEG = 0  # 最小仰角（地平线以上）
MAX_DIST_KM = 2000  # 最大距离（2000km）
MIN_ALT_DEG = _env_float("S1_MIN_ALT_DEG", MIN_ALT_DEG)  # 最小仰角（地平线以上）
MAX_DIST_KM = _env_float("S1_MAX_DIST_KM", MAX_DIST_KM)  # 最大距离（2000km）
MAX_SAT_COUNT = _env_int("S1_MAX_SAT_COUNT", 25)  # 最终输出卫星数量
IP_PREFIX = _env_str("S1_IP_PREFIX", "10.0.3.")  # 卫星IP前缀
...
ROOT_OUTPUT_DIR = os.path.join(PARENT_DIR, "output")
...
CHUNK_DURATION_SEC = _env_int("S1_CHUNK_DURATION_SEC", 60)  # 每个文件的时间切片（60秒）

# 5. 动态筛选配置
DYNAMIC_FILTER_INTERVAL_SEC = _env_int("S1_DYNAMIC_FILTER_INTERVAL_SEC", 60)  # 动态筛选时间窗口（每60秒重新筛选一次）
RESELECT_SAT_COUNT = _env_int("S1_RESELECT_SAT_COUNT", 25)  # 每次动态筛选保留的卫星数
REFRESH_TLE = _env_int("S1_REFRESH_TLE", 1)  # 启动时是否尝试实时更新TLE（1=是，0=否）
...
def split_and_save_csv(trajectory_df):
    ...
    manifest_path = os.path.join(ROOT_OUTPUT_DIR, "manifest.json")
    import json
    with open(manifest_path, "w", encoding="utf-8") as f:
        json.dump(manifest, f, indent=2)
    print("📋 生成索引文件：manifest.json")
...
if __name__ == "__main__":
    try:
        ...
        if REFRESH_TLE:
            print("🌐 正在尝试更新最新 TLE 数据...")
            if not download_latest_tle():
                exit(1)
        else:
            print(f"📦 使用本地 TLE 文件：{TLE_FILE}")
